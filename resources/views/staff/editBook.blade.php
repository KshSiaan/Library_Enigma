<x-staff-layout>
    <form method="POST" action="{{ route('staff.manageBooks.update', ['id' => $book->id]) }}"
        enctype="multipart/form-data">
        @csrf
        <div class="p-4">
            <h1 class="font-medium text-xl text-center">Add a book to the library database</h1>
            <div class="container flex flex-row justify-end"><button type="submit"
                    class="btn btn-success bg-success">Update
                    Book</button></div>
                                ...
                                <label for="cover"
                                    ...
                                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                        ...
                                        <p id="file-name" class="mb-2 text-sm text-gray-900"><span
                                                class="font-semibold">{{ str_replace('uploads/covers/', '', $book->cover) }}
                                    ...
                                    <input id="cover" name="cover" type="file" class="hidden"
                                        src="{{ $book->cover }}" onchange="updateFileName(this)" />
                                </label>
                                    ...
                                    <select name="author_id" id="author_id"
                                        ...
                                        @foreach ($authors as $author)
                                            @if ($author->id == $book->author_id)
                                                <option value="{{ $author->id }}" selected>{{ $author->author }}
                                                </option>
                                            @endif
                                ...
                                <div class="col-4">
                                    ...
                                    <select name="catagory_id" id="catagory_id"
                                        ...
                                        @foreach ($catagories as $catagory)
                                            @if ($catagory->id == $book->catagory_id)
                                                <option value="{{ $catagory->id }}" selected>{{ $catagory->catagory }}
                                                </option>
                                            @endif
                    ...
                    <div class="container">
                        <div class="w-full mx-auto">
                            ...
                            <textarea type="text" name="book_detail" id="book_detail"
                                class="w-full rounded-md border border-[#e0e0e0] bg-white py-2 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
                                rows="10" spellcheck="true" style="resize: vertical">{{ $book->book_detail }}</textarea>
